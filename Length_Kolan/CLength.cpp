// CLength.cpp : Defines the entry point for the console application.
//
...
void Out_File(char* in_filename,char* out_filename)
{
	...
	out_file << "nkangus1991@163.com" << endl;
	out_file << endl;
	string line;
	char out_format[128];
	while (getline(in_file,line))
	{
		if (line.find("=") == string::npos && line.length() > 0)
		{
			...
			{
				//cout << elements[i].base << "\t" << elements[i].times << endl;
